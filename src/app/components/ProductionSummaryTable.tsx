...
import { Package, CheckCircle, XCircle, Box, Component, Puzzle, ArrowRightCircle } from 'lucide-react';
...
interface ProductionSummaryTableProps {
  summary: SummaryItem[];
  isLoading: boolean;
  onUseStock: (item: SummaryItem) => void; // New prop for handling stock usage
}
            ...
            {summary.length > 0 ? (
              summary.map((item, index) => (
                <tr key={`${item.sku}-${index}`}>
                  <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500">{item.sku}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.produtoNome}</td>
              ...
              <tr>
                <td colSpan={11} className="text-center py-4 text-sm text-gray-500">
