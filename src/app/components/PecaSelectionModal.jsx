...
const PecaSelectionModal = ({ isOpen, onClose, onSelectPecas, selectedPecas: initialSelectedPecas }) => {
  const [availablePecas, setAvailablePecas] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedPecas, setSelectedPecas] = useState(initialSelectedPecas || []);

  useEffect(() => {
    const fetchAvailablePecas = async () => {
      try {
        const pecasCollection = collection(db, 'pecas'); // Assuming 'pecas' is the collection name
        const pecaSnapshot = await getDocs(pecasCollection);
        const pecasList = pecaSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    if (isOpen) {
      fetchAvailablePecas();
      setSelectedPecas(initialSelectedPecas || []); // Reset selected pecas when opening
    }
  }, [isOpen, initialSelectedPecas]);

  const filteredPecas = availablePecas.filter(peca =>
    peca.nome.toLowerCase().includes(searchTerm.toLowerCase()) ||
    peca.sku.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleCheckboxChange = (peca) => {
    setSelectedPecas(prev => {
      if (prev.some(p => p.id === peca.id)) {
        return prev.filter(p => p.id !== peca.id);
      } else {
        return [...prev, { ...peca, quantity: 1 }]; // Add with default quantity 1
      }
    });
  };

  const handleQuantityChange = (pecaId, quantity) => {
    setSelectedPecas(prev => prev.map(peca =>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredPecas.length === 0 ? (
                ...
                filteredPecas.map(peca => {
                  const isSelected = selectedPecas.some(p => p.id === peca.id);
                  const currentQuantity = isSelected ? selectedPecas.find(p => p.id === peca.id).quantity : 1;
                  return (
                    <tr key={peca.id}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => handleCheckboxChange(peca)}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {isSelected && (
                          <input
                            ...
                            onChange={(e) => handleQuantityChange(peca.id, e.target.value)}
...
export default PecaSelectionModal;
