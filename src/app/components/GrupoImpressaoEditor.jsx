...
export default function GrupoImpressaoEditor({
  ...
  const handleInsumoChange = (index, field, value, alternativeIndex = null) => {
    const updatedInsumos = JSON.parse(JSON.stringify(grupo.filamentos || [])); // Renamed from filamentos to insumos

    if (alternativeIndex === null) {
      // It's a main insumo
      updatedInsumos[index][field] = value;
    } else {
      // It's an alternative insumo (only for filaments currently)
      if (!updatedInsumos[index].alternativeFilaments) {
        updatedInsumos[index].alternativeFilaments = [];
      }
      updatedInsumos[index].alternativeFilaments[alternativeIndex][field] = value;
    }

    onChange({ ...grupo, filamentos: updatedInsumos }); // Still using 'filamentos' property in GrupoImpressao
  };
  ...
  const removeInsumo = (index, alternativeIndex = null) => {
    const updatedInsumos = JSON.parse(JSON.stringify(grupo.filamentos || []));

    if (alternativeIndex === null) {
      // Remove main insumo
      updatedInsumos.splice(index, 1);
    } else {
      // Remove alternative insumo
      if (updatedInsumos[index] && updatedInsumos[index].alternativeFilaments) {
        updatedInsumos[index].alternativeFilaments.splice(alternativeIndex, 1);
      }
    }
    onChange({ ...grupo, filamentos: updatedInsumos });
  };
  ...
  return (
    <div className="border rounded-lg p-4 mb-4 bg-gray-50">
      ...
      <div className="space-y-6">
        {/* Insumos Section */}
        <div>
          <h5 className="text-sm font-medium text-gray-700 mb-2">Insumos</h5>
          <div className="space-y-3">
            {(grupo.filamentos || []).map((insumo, index) => (
              <div key={index} className="bg-white p-3 rounded-md border">
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => handleInsumoChange(index, 'grupoFilamentoId', e.target.value)}
                      ...
                      {insumo.tipo === 'material' && (
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => handleInsumoChange(index, 'etapaInstalacao', e.target.value)}
                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm"
                          >
                ...
                {/* Alternative Filaments (only for filament type) */}
                {insumo.tipo === 'filamento' && (
                    ...
                    {(insumo.alternativeFilaments || []).map((altFil, altIndex) => (
                      <div key={altIndex} className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) => handleInsumoChange(index, 'grupoFilamentoId', e.target.value, altIndex)}
                        ...
                        <button type="button" onClick={() => removeInsumo(index, altIndex)} className="text-red-500 hover:text-red-700 p-1">
                ...
                <div className="flex justify-end mt-2">
                  <button type="button" onClick={() => removeInsumo(index)} className="text-red-500 text-xs hover:text-red-700">
                    Remover Insumo
                  </button>
                </div>
              </div>
            ...
            <div className="flex space-x-2 mt-2">
              {/* Botões para adicionar diferentes tipos de insumos */}
              <button
                type="button"
