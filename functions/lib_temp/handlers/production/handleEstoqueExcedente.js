...
import { LancamentoProducaoTipoEvento } from '../../types/productionTypes';
import { logger } from 'firebase-functions';
export const handleEstoqueExcedente = async (snapshot) => {
    const db = admin.firestore();
    const lancamento = snapshot.data();
    if (lancamento.tipoEvento !== LancamentoProducaoTipoEvento.ESTOQUE_EXCEDENTE) {
        return null;
    }
    const payload = lancamento.payload;
    const { produtoId, produtoTipo, quantidade, localId, recipienteId, divisao, observacao } = payload;
    logger.info(`Processando estoque_excedente para produto: ${produtoId} (${produtoTipo}), quantidade: ${quantidade}`);
    try {
        ...
        logger.error(`Erro ao processar estoque_excedente para lancamentoId: ${snapshot.id}`, error);
        throw error;
    }
    return null;
};
